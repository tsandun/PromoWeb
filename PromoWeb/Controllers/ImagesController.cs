...
namespace PromoWeb.Controllers
{
    public class ImagesController : ApiController
    {
        ...
        // GET: api/Images/AdvertisementId/
        [ResponseType(typeof(Image))]
        public List<Image> GetImageByAdvertisement(int id)
        {
            List<Image> images = new List<Image>();

            var checkId = db.Addresses.Find(id);
            if (checkId != null)
            {
                var image = db.Images.Where(x => x.Advertisement.Id == id).ToList();
                images = image;
            }
            else
            {
                throw new HttpResponseException(Request.CreateErrorResponse(HttpStatusCode.NotFound,"Advertisement does not exists"));
            }
